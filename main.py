...
# Function to get AI-generated report
...
# Function to extract scores from report
...
# -------- Function to get Added Information --------
def get_added_information(resume, job_desc):
    """
    Uses LLM to generate additional role-specific points based on the job description
    that could be added to the candidate's resume.
    """
    client = Groq(api_key=api_key)

    prompt = f"""
    You are an AI assistant helping candidates enhance their resumes.
    Based on the following job description, suggest additional, relevant points,
    skills, or experiences that can be added to their resume to make it stronger.

    # Instructions:
    - Focus strictly on the given job description.
    - Include only points that are professional and resume-appropriate.
    - Avoid repetition of points already present in the resume.
    - Present the suggestions in bullet points.
    ...
    - Bullet points with actionable and relevant content.
    """
...
if st.session_state.form_submitted:
    ...
    st.subheader("Added information:")
    added_info = get_added_information(st.session_state.resume, st.session_state.job_desc)

    st.markdown(f"""
        <div style='text-align: left; background-color: #1a1a1a; padding: 10px; border-radius: 10px; margin: 5px 0;'>
        {added_info}
        </div>
        """, unsafe_allow_html=True)

    # Download Added Information
    st.download_button(
        label="Download Added Information",
        data=added_info,
        file_name="added_information.txt",
        icon=":material/download:",
    )
